...
def program(dbClient, port=None, baudrate=None):
    ...
    while not finished:
        ...
        if (length > 0):
            ...
            print(f'deviceID: {chr(deviceID[0])}, counter: {counter}, txMode: {txMode}, txCounter: {txCounter}, csmaRetries: {csmaRetries}, csmaRSSI: {csmaRSSI}, rssi: {rssi}, pkt_type: {pkt_type}')#\n\tRX bad {statistics["rx_bad_frames"]}, RX good {statistics["rx_good_frames"]}, RX total {statistics["rx_total_frames"]}')
